...
public interface CatDao {
    ...
    @Select("<script>" +
            ...
            "<if test='isHot != null and isHot == true'>AND is_hot = 1</if>" +
            "<if test='search != null and search != \"\"'>AND (name LIKE CONCAT('%', #{search}, '%') OR breed LIKE CONCAT('%', #{search}, '%'))</if>" +
            "</where>" +
    ...
    @Select("SELECT * FROM cat WHERE add_user_id = #{addUserId} ORDER BY create_time DESC")
    List<Cat> findByAddUserId(Long addUserId);

    @Select("SELECT * FROM cat ORDER BY create_time DESC")
    List<Cat> findAll();
