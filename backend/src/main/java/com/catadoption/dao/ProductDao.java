...
public interface ProductDao {
    ...
    @Insert("INSERT INTO product(name, category, brand, price, stock, image_url, image_urls, description, status) " +
            "VALUES(#{name}, #{category}, #{brand}, #{price}, #{stock}, #{imageUrl}, #{imageUrls}, #{description}, 'available')")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(Product product);
    
    @Update("UPDATE product SET name=#{name}, category=#{category}, brand=#{brand}, price=#{price}, " +
            "stock=#{stock}, image_url=#{imageUrl}, image_urls=#{imageUrls}, description=#{description}, status=#{status} " +
            "WHERE id=#{id}")
