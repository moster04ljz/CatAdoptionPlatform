      ...
      <el-row :gutter="30">
        ...
        <el-col :span="12">
          <el-card class="info-card">
            <div class="header">
              <h1>{{ cat.name }}</h1>
              <el-tag :type="cat.category === 'market' ? 'warning' : 'success'" size="large">
                {{ cat.category === 'market' ? '商城出售' : '免费领养' }}
              </el-tag>
            </div>
            
            <div class="price" v-if="cat.price > 0">¥{{ cat.price }}</div>
            <div class="price free" v-else>免费领养</div>
            ...
            <div class="actions" v-if="user.id && cat.status === 'available'">
              <el-button v-if="cat.category === 'adoption'" type="primary" size="large" @click="showApply = true">
                申请领养
              </el-button>
              <el-button v-else type="warning" size="large" @click="handleBuy">
                立即购买
              </el-button>
            </div>
      ...
      <el-form :model="applyForm" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="居住情况" prop="housing">
          <el-select v-model="applyForm.housing" placeholder="请选择">
            ...
            <el-option label="与父母同住" value="与父母同住" />
          </el-select>
        </el-form-item>
        <el-form-item label="养猫经验" prop="experience">
          <el-input v-model="applyForm.experience" type="textarea" :rows="2" />
        </el-form-item>
...
.header h1 { margin: 0; }
.price { font-size: 28px; font-weight: bold; color: #e6a23c; margin: 15px 0; }
