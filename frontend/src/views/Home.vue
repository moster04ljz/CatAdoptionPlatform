  ...
  <div class="home">
        ...
        <template v-if="user.id">
          ...
          <el-dropdown @command="handleCommand">
            <span class="user-dropdown">
              <el-avatar :size="28" :src="user.avatar">{{ user.nickname?.[0] }}</el-avatar>
    ...
    <div class="carousel-container">
      <el-carousel height="400px" :interval="5000" indicator-position="outside">
        <el-carousel-item v-for="(item, idx) in carousels" :key="idx">
          <div class="carousel-item-wrapper" @click="goCarouselCat(item)">
            <img :src="item.imageUrl || defaultCarousel" class="carousel-img" />
            <div class="carousel-caption">
              <h2>{{ item.title || '让爱流动 · 领养代替购买' }}</h2>
              <p v-if="item.catId">点击查看详情 →</p>
              <p v-else>每一个生命都值得被温柔以待</p>
            </div>
          </div>
        </el-carousel-item>
      </el-carousel>
    ...
    <div class="section" v-if="hotCats.length > 0">
      ...
      <div class="cat-grid">
        <el-card v-for="cat in hotCats" :key="cat.id" class="cat-card" shadow="hover" @click="goDetail(cat.id)">
          ...
          <div class="cat-info">
            <h3>{{ cat.name }}</h3>
            <p class="publisher" v-if="cat.addUserName">{{ cat.addUserRole === 'admin' ? '平台发布' : `寄养人：${cat.addUserName}` }}</p>
            <div class="tags">
              <el-tag size="small">{{ cat.age }}岁</el-tag>
              <el-tag size="small">{{ cat.gender }}</el-tag>
    ...
    <div class="section">
      ...
      <div class="filter-bar">
        <el-input v-model="search" placeholder="搜索猫咪名字/品种..." clearable @keyup.enter="loadCats" style="width: 250px">
          <template #append><el-button :icon="Search" @click="loadCats" /></template>
        </el-input>
        <el-button type="primary" @click="$router.push('/market')">查看全部 >></el-button>
      </div>
      <div class="cat-grid">
...
import { getCatList, getCarousels, getUnreadMessages } from '../api'
...
const loadCats = async () => {
  const res = await getCatList(search.value)
  if (res.data.code === 200) {
    const allCats = res.data.data
    hotCats.value = allCats.filter(c => c.isHot && c.status === 'available').slice(0, 4)
    adoptionCats.value = allCats.filter(c => c.status === 'available').slice(0, 8)
  }
}
...
const loadUnread = async () => {
  ...
  try {
    ...
    if (res.data.code === 200) unreadCount.value = res.data.data.length
  } catch { /* ignore */ }
}

const goDetail = (id) => router.push(`/cat/${id}`)

const goCarouselCat = (item) => {
  if (item.catId) router.push(`/cat/${item.catId}`)
}
...
.navbar { display: flex; justify-content: space-between; align-items: center; padding: 12px 30px; background: white; box-shadow: 0 2px 12px rgba(0,0,0,0.08); position: sticky; top: 0; z-index: 100; }
...
.carousel-caption { position: absolute; bottom: 60px; left: 60px; color: white; text-shadow: 2px 2px 4px rgba(0,0,0,0.5); }
.carousel-caption h2 { font-size: 36px; margin: 0 0 10px; }
.carousel-caption p { font-size: 18px; margin: 0; }
.carousel-item-wrapper { cursor: pointer; position: relative; height: 100%; }
.carousel-item-wrapper:hover .carousel-img { transform: scale(1.02); transition: transform 0.3s; }
...
.publisher { color: #409eff; font-size: 13px; margin: 4px 0; }
.tags { display: flex; gap: 5px; margin-bottom: 8px; flex-wrap: wrap; }
